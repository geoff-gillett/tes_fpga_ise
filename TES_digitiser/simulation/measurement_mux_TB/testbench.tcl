...
wave add /measurement_mux_TB/\\changen(0)\\/measure
wave add /measurement_mux_TB/\\changen(1)\\/measure

runclks
set i 0

writeInt32 $settings0 registers(0).dsp.baseline.subtraction
writeInt32 $settings1 registers(1).dsp.baseline.subtraction
writeInt32 $settings0 registers(0).dsp.baseline.offset
writeInt32 $settings1 registers(1).dsp.baseline.offset
writeInt32 $settings0 registers(0).dsp.constant_fraction
writeInt32 $settings1 registers(1).dsp.constant_fraction
writeInt32 $settings0 registers(0).dsp.pulse_threshold
writeInt32 $settings1 registers(1).dsp.pulse_threshold
writeInt32 $settings0 registers(0).dsp.slope_threshold 
writeInt32 $settings1 registers(1).dsp.slope_threshold 
writeInt32 $settings0 registers(0).dsp.baseline.average_order 
writeInt32 $settings1 registers(1).dsp.baseline.average_order 
writeInt32 $settings0 \\changen(0)\\/measure/SignalProcessor/BASELINE_AV_FRAC 
writeInt32 $settings1 \\changen(1)\\/measure/SignalProcessor/BASELINE_AV_FRAC 
writeInt32 $settings0 registers(0).capture.rel_to_min
writeInt32 $settings1 registers(1).capture.rel_to_min
writeInt32 $settings0 registers(0).capture.use_cfd_timing
writeInt32 $settings1 registers(1).capture.use_cfd_timing
writeInt32 $settings0 height_slvs(0)
writeInt32 $settings1 height_slvs(1)
close $settings0
close $settings1
...
#while {$i < 500000} {}
  #gets $fp hexsample
	...
	if [getsig measurements(1).pulse.valid] {
		puts -nonewline $pulsemeasurements1 [binary format i $i]
		writeInt32 $pulsemeasurements1 measurements(0).pulse.area
		writeInt32 $pulsemeasurements1 measurements(0).pulse.extrema
	}
	...
	if { [getsig ready] && [getsig valid] } {
		puts "sample $i:stream [getsig outstream.data hex]"
		writeInt32 $stream outstream.data(31:0)
		writeInt32 $stream outstream.data(63:32)
	}
...
close $pulsemeasurements0 
close $pulsemeasurements1 
